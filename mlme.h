...
typedef struct _RT_PHY_INFO{
	unsigned char		bHtEnable;	 /* If we should use ht rate. */
	unsigned char		bPreNHt;	 /* If we should use ht rate. */
	/*Substract from HT Capability IE */
	unsigned char		MCSSet[16];
#ifdef DOT11_VHT_AC
	unsigned char 	bVhtEnable;
...
typedef	struct _CIPHER_SUITE {
	...
	unsigned char							bMixMode;		/* Indicate Pair & Group cipher might be different */
...
/*  unsigned char     bMoreDataAck; */
    ...
    unsigned char     bACM[4];      /* 1: Admission Control of AC_BK is mandattory */
...
typedef struct {
    unsigned char       IELen;
    unsigned char       IE[MAX_CUSTOM_LEN];
} WPA_IE_;
#endif /* CONFIG_STA_SUPPORT */
...
typedef struct {
	...
	unsigned char		CountryString[3];
	unsigned char		bHasCountryIE;
