...
static void andes_init_cmd_msg(struct cmd_msg *msg, u8 type, unsigned char need_wait, u16 timeout, 
							   unsigned char need_retransmit, unsigned char need_rsp, u16 rsp_payload_len, 
...
unsigned char is_inband_cmd_processing(RTMP_ADAPTER *ad)
{
	unsigned char ret = 0;
...
int andes_send_cmd_msg(PRTMP_ADAPTER ad, struct cmd_msg *msg)
{
	struct MCU_CTRL *ctl = &ad->MCUCtrl;
	int ret = 0;
	unsigned char need_wait = msg->need_wait;
...
int andes_burst_write(RTMP_ADAPTER *ad, u32 offset, u32 *data, u32 cnt)
{
	...
	RTMP_CHIP_CAP *cap = &ad->chipCap;
	int ret = 0;
	unsigned char last_packet = FALSE;
