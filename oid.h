...
typedef struct __attribute__ ((packed)) _DOT1X_IDLE_TIME{
...
typedef struct {
	unsigned char bValid;		/* 1: variable contains valid value */
...
typedef struct _OID_BACAP_STRUC {
	...
	unsigned char AutoBA;		/* Auto BA will automatically */
...
typedef struct _NINTENDO_SSID {
	unsigned char NINTENDOFixchar[NINTENDO_SSID_NAME_LN];
	unsigned char zero1;
	unsigned char registe;
	unsigned char ID;
	unsigned char zero2;
	unsigned char NICKname[NINTENDO_SSID_NICKNAME_LN];
} RT_NINTENDO_SSID, *PRT_NINTENDO_SSID;

typedef struct _NINTENDO_ENTRY {
	unsigned char NICKname[NINTENDO_SSID_NICKNAME_LN];
	unsigned char DS_Addr[ETH_LENGTH_OF_ADDRESS];
	unsigned char registe;
	unsigned char UserSpaceAck;
...
typedef struct _NINTENDO_SEED_WEPKEY {
	unsigned char seed[NINTENDO_SSID_NICKNAME_LN];
	unsigned char wepkey[16];	/*use 13 for 104 bits wep key */
...
struct bbp_info {
	unsigned int bbp_start;
	unsigned int bbp_end;
	unsigned char bbp_value[0];
};

struct bbp32_info {
	unsigned int bbp_start;
	unsigned int bbp_end;
	unsigned int bbp_value[0];
};

struct rf_info {
	unsigned short rf_start;
	unsigned short rf_end;
	unsigned char rf_value[0];
};

struct rf_bank_info {
	unsigned char rf_bank;
	unsigned short rf_start;
	unsigned short rf_end;
	unsigned char rf_value[0];
};
...
struct mac_info {
	unsigned int mac_start;
	unsigned int mac_end;
	unsigned int mac_value[0];
};

struct mem_map_info {
	unsigned int base;
	unsigned short mem_map_start;
	unsigned short mem_map_end;
	unsigned int mem_map_value[0];
};

struct e2p_info {
	unsigned short e2p_start;
	unsigned short e2p_end;
...
struct phy_mode_info {
	...
	unsigned char data_ldpc;
	unsigned char data_mcs;
	unsigned char data_gi;
	unsigned char data_stbc;
};
