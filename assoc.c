...
unsigned char CipherWpaTemplate[] = {
	0xdd,			/* WPA IE */
	0x16,			/* Length */
...
unsigned char CipherWpa2Template[] = {
	0x30,			/* RSN IE */
	0x14,			/* Length */
	0x01, 0x00,		/* Version */
...
MLME_QUEUE_ELEM *Elem)
{
	...
	else if (MlmeAssocReqSanity(pAd, Elem->Msg, Elem->MsgLen, ApAddr, &CapabilityInfo, &Timeout, &ListenIntv))
	{
		...
		if ((pAd->MlmeAux.HtCapabilityLen > 0)
			...
			unsigned char HtLen, BROADCOM[4] = { 0x0, 0x90, 0x4c, 0x33 };
			...
			*(unsigned short *) (&HtCapabilityTmp.ExtHtCapInfo) = SWAP16(*(unsigned short *) (&HtCapabilityTmp.ExtHtCapInfo));
			...
			{
				FrameLen += build_vht_ies(pAd, (unsigned char *)(pOutBuffer + FrameLen), SUBTYPE_ASSOC_REQ);
			}
#endif /* DOT11_VHT_AC */
		}
...
MLME_QUEUE_ELEM * Elem)
{
	...
	else if (MlmeAssocReqSanity(pAd, Elem->Msg, Elem->MsgLen, ApAddr, &CapabilityInfo, &Timeout, &ListenIntv))
	{
		...
		if (pAd->CommonCfg.bAggregationCapable) {
			...
			unsigned long TmpLen;
			unsigned char RalinkIe[9] = {IE_VENDOR_SPECIFIC, 7, 0x00, 0x0c, 0x43, 0x04, 0x00, 0x00, 0x00 };
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, 9,
					  RalinkIe, END_OF_ARGS);
			FrameLen += TmpLen;
		}
...
MLME_QUEUE_ELEM *Elem)
{
	...
	unsigned char HtCapabilityLen = 0;
	...
	os_alloc_mem(pAd, (unsigned char **)&ie_list, sizeof(IE_LISTS));
	if (ie_list == NULL) {
		DBGPRINT(RT_DEBUG_OFF, ("%s():mem alloc failed!\n", __FUNCTION__));
		return;
	}
	NdisZeroMemory((unsigned char *)ie_list, sizeof(IE_LISTS));
...
unsigned short CapabilityInfo)
{
	...
	unsigned char bSupportN = FALSE;
#ifdef TXBF_SUPPORT
	unsigned char		supportsETxBf = FALSE;
#endif
