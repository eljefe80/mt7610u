...
typedef union _BBP_R49_STRUC {
	struct
	{
		unsigned char	adc5_in_sel:1; /* 0: TSSI (from the external components, old version), 1: PSI (int ernal components, new version - RT3390) */
...
/* */
/* BBP & RF are using indirect access. Before write any value int o it. */
...
int  NICInitBBP(struct _RTMP_ADAPTER *pAd);
