...
VOID RtmpAsicSleepHandle(
    IN  PRTMP_ADAPTER       pAd)
{
	unsigned char FlgCanAsicSleep = TRUE;
...
========================================================================
*/
unsigned char UAPSD_SP_IsClosed(
    IN  PRTMP_ADAPTER       pAd,
	IN	MAC_TABLE_ENTRY		*pEntry)
{
	unsigned char FlgIsSpClosed = TRUE;
...
VOID UAPSD_AssocParse(
	...
	IN	MAC_TABLE_ENTRY		*pEntry,
	IN	UCHAR				*pElm,
	IN	unsigned char				FlgApsdCapable)
{
	...
	IN	MAC_TABLE_ENTRY		*pEntry)
{
	QUEUE_HEADER *pQue;
	UINT32 IdAc;
	unsigned char FlgUapsdPkt, FlgEospPkt;
#ifdef RTMP_MAC_USB
	ULONG IrqFlags;
#endif /* RTMP_MAC_USB */
...
========================================================================
*/
unsigned char UAPSD_PsPollHandle(
	IN	PRTMP_ADAPTER		pAd,
	IN	MAC_TABLE_ENTRY		*pEntry)
{
...
VOID UAPSD_QueueStatusGet(
	IN	PRTMP_ADAPTER		pAd,
	IN	MAC_TABLE_ENTRY		*pEntry,
	OUT	unsigned char				*pFlgIsAnyPktForBK,
	unsigned char				*pFlgIsAnyPktForBE,
	unsigned char				*pFlgIsAnyPktForVI,
	unsigned char				*pFlgIsAnyPktForVO)
{
	/* init */
	...
	IN	UCHAR				UpOfFrame)
{
	...
	*/
	...
	UCHAR	QueIdList[WMM_NUM_OF_AC] = { QID_AC_BE, QID_AC_BK,
                                            QID_AC_VI, QID_AC_VO };
	unsigned char	FlgQueEmpty;
	unsigned char	FlgNullSnd;
	unsigned char	FlgMgmtFrame;
	UINT32	Aid, QueId;
	INT32	IdAc; /* must be signed, can not use unsigned */
/*	ULONG    FlgIrq; */
