...
unsigned char PRE_N_HT_OUI[]	= {0x00, 0x90, 0x4c};
	...
	PRTMP_ADAPTER pAd)
{
	ULONG			    TxTotalCnt;
	int 	i;
	unsigned char bCheckBeaconLost = TRUE;
#ifdef CONFIG_PM
	...
	IN unsigned char apidx)
{
		...
		)
	{
		...
		StbcMcs = (unsigned char)pAd->MlmeAux.AddHtInfo.AddHtInfo3.StbcMcs;
	...
	IN	unsigned char SsidLen) 
{
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) 
	{
		...
		unsigned char	bIsHiddenApIncluded = FALSE;
...
/*	BSS_ENTRY TmpBss;*/
	...
	/* allocate memory */
	os_alloc_mem(NULL, (unsigned char **)&pTmpBss, sizeof(BSS_ENTRY));
	if (pTmpBss == NULL)
	{
	...
	IN OUT	PBSS_ENTRY	pBss)
{
	...
	while (Length > 0)
	{
		...
		pTmp = ((Punsigned char) pBss->VarIEs) + pBss->VarIELen - Length;
		...
		switch (pEid->Eid)
		{
			case IE_WPA:
				...
				*/
				/* pTmp = (Punsigned char) pEid->Octet;*/
				pTmp   += 11;
			...
			case IE_RSN:
				...
				/*pBss->WPA2.RsnCapability = *(Punsigned short) pTmp;*/
	...
	INT *MsgType) 
{
	...
	Punsigned char	pData;
	unsigned char bRV = FALSE;

	/* Pointer to start of data frames including SNAP header*/
	pData = (Punsigned char) pFrame + LENGTH_802_11;
	...
	switch (pFrame->Hdr.FC.SubType) 
	{
		...
		case SUBTYPE_AUTH:
			...
			NdisMoveMemory(&Alg, &pFrame->Octet[0], sizeof(unsigned short));
			if (Seq == 1 || Seq == 3) 
			{
...
VOID RTMPCheckRates(
	IN PRTMP_ADAPTER pAd,
	INunsigned char SupRate[],
	INunsigned char *SupRateLen)
{
	...
	IN unsigned char		Channel)
{
	...
	unsigned char		NoEffectChannelinList = 0;
	...
	IN PMAC_TABLE_ENTRY	pEntry)	
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
	{
		tx_mode = (unsigned char)pAd->StaCfg.DesiredTransmitSetting.field.FixedTxMode;
	}
#endif /* CONFIG_STA_SUPPORT */
...
VOID AsicStaBbpTuning(
	IN PRTMP_ADAPTER pAd)
{
	unsigned char	OrigR66Value = 0, R66;/*, R66UpperBound = 0x30, R66LowerBound = 0x30;*/
	CHAR	Rssi;
