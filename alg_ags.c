...
void MlmeDynamicTxRateSwitchingAGS(
	...
	IN unsigned char TableSize, 
	IN PAGS_STATISTICS_INFO pAGSStatisticsInfo,
	IN unsigned char InitTxRateIdx)
{
	unsigned char UpRateIdx = 0, DownRateIdx = 0, CurrRateIdx = 0;
	char bTxRateChanged = TRUE, bUpgradeQuality = FALSE;
	...
	do
	{
		...
		pEntry->PER[CurrRateIdx] = (unsigned char)(pAGSStatisticsInfo->TxErrorRatio);
...
{
	unsigned char UpRateIdx = 0, DownRateIdx = 0, CurrRateIdx = 0;
	char bTxRateChanged = TRUE;
	RTMP_RA_AGS_TB *pCurrTxRate = NULL;
	RTMP_RA_LEGACY_TB *pNextTxRate = NULL;
	unsigned char TrainDown = 0, TrainUp = 0;
	char ratio = 0;
