...
typedef	union _PROT_CFG_STRUC {
	struct {
#ifdef DOT11_VHT_AC
		unsigned int		 ProtectTxop:3; /* TXOP allowance */
		unsigned int       DynCbw:1;  /* RTS use dynamic channel bandwidth when TX signaling mode is turned on */
		unsigned int       RtsTaSignal:1; /* RTS TA signaling mode */
#else
	    unsigned int       rsv:5;	 
#endif
...
int get_pkt_snr_by_rxwi(RXWI_STRUC *rxwi, int size, unsigned char *snr);
