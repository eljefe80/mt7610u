...
typedef struct _TX_CONTEXT
{
	...
	char			InUse;
	char			bWaitingBulkOut; /* at least one packet is in this TxContext, ready for making IRP anytime. */
	char			bFullForBulkOut; /* all tx buffer are full , so waiting for tx bulkout. */
	char			IRPPending;
	char			LastOne;
	char			bAggregatible;
...
typedef struct _RX_CONTEXT
{
	...
	PURB				pUrb;
	/*These 2 char shouldn't both be 1 at the same time. */
	unsigned long				BulkInOffset;	/* number of packets waiting for reordering . */
/*	char				ReorderInUse;	// At least one packet in this buffer are in reordering buffer and wait for receive indication */
	char				bRxHandling;	/* Notify this packet is being process now. */
	char				InUse;			/* USB Hardware Occupied. Wait for USB HW to put packet. */
	char				Readable;		/* Receive Complete back. OK for driver to indicate receiving packet. */
	char				IRPPending;		/* TODO: To be removed */
	/*atomic_t				IrpLock; */
