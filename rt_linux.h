...
#define RTMP_IO_FORCE_READ32(_A, _R, _pV)								\
	RTUSBReadMACRegister((_A), (_R), (Punsigned int) (_pV))
...
#define GET_OS_PKT_DATATAIL(_pkt) \
        (RTPKT_TO_OSPKT(_pkt)->head + (unsigned long)RTPKT_TO_OSPKT(_pkt)->tail)
#define SET_OS_PKT_DATATAIL(_pkt, _start, _len) \
        ((RTPKT_TO_OSPKT(_pkt))->tail) = (unsigned long)_start - (unsigned long)(RTPKT_TO_OSPKT(_pkt)->head) + (_len)
#else
#define GET_OS_PKT_DATATAIL(_pkt) \
		(RTPKT_TO_OSPKT(_pkt)->tail)
#define SET_OS_PKT_DATATAIL(_pkt, _start, _len)	\
		((RTPKT_TO_OSPKT(_pkt))->tail) = (unsigned long)((_start) + (_len))
#endif 
